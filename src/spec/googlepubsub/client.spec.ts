import {GoogleClient, NoHanlderToProcessEvents} from '../../googlepubsub/client';
import {
	createSpiedBackoff,
...
describe('GoogleBus Client', () => {
	...
	it('should call the getEvents from the eventstore library', (done) => {
		...
		const client = new GoogleClient(mockedSpiedEventstore, eventstoreSignal, errorLogger, spiedBackoff, decodeEventstoreResponse, [{stream: 'a', subscription: 'a'}], tracker, 50);
	...
	it('should do one call for each iteration with the correct backoff', (done) => {
		...
		const mockedSpiedEventstore = createMockedSpiedEventstorelibWithNoEvents();
		const client = new GoogleClient(mockedSpiedEventstore, eventstoreSignal, errorLogger, backoff, decodeEventstoreResponse, [{stream: 'a', subscription: 'a'}], tracker, 50);
		client.startConsumption();
	...
	it('should stop once all events are processed', (done) => {
		...
		const mockedSpiedEventstore = createMockedSpiedEventstorelibWithCorrectEvents(1);
		const client = new GoogleClient(mockedSpiedEventstore, eventstoreSignal, errorLogger, spiedBackoff, decodeEventstoreResponse, [{stream: 'a', subscription: 'a'}], tracker, 50000000);
		client.startConsumption();
